...
class tablas {
	...
	public static function getSalas($edificioid = null) {
		...
		foreach ( $salas as $sala ) {
			...
			if ($edificioid) {
				$editurl_sala = new moodle_url ( 'salas.php', array (
						'action' => 'editar',
						'prevaction' => 'verporedificio',
						'idsala' => $sala->id,
						'sesskey' => sesskey () 
				) );
				$deleteurl_sala = new moodle_url ( 'salas.php', array (
						'action' => 'borrar',
						'prevaction' => 'verporedificio',
	...
	public static function datosSedesEdificios() {
		...
		$tabla->head = array (
				get_string ( 'campus', 'local_reservasalas' ),
				get_string ( 'building', 'local_reservasalas' ),
				get_string ( 'seechangerooms', 'local_reservasalas' ),
				get_string ( 'deletebuilding', 'local_reservasalas' ) 
		);
		...
		foreach ( $edificios as $edificio ) {
			...
			$editurl_edificios = new moodle_url ( 'salas.php', array (
			...
			$deleteurl_sedes = new moodle_url ( 'edificios.php', array (
					'action' => 'borrar',
					'edificio' => $edificio->id,
					'sesskey' => sesskey () 
			) );
	...
	// Used in edificios.php, shows all the existing buildings.
	public static function datasPlacesBuildingsAdminRoom() {
		...
		foreach ( $edificios as $edificio ) {
			...
			$editurl_edificios = new moodle_url ( 'edificios.php', array (
					'action' => 'editar',
					'edificio' => $edificio->id 
			) );
			...
			// CHANGE URL
			$seeurl_salas = new moodle_url ( 'salas.php', array (
					'action' => 'verporedificio',
					'edificio' => $edificio->id,
					'sesskey' => sesskey () 
			) );
	...
	public static function getSedes() {
		...
		$tabla->head = array (
				get_string ( 'campus', 'local_reservasalas' ),
				get_string ( 'deletecampus', 'local_reservasalas' ) 
		);
	...
	// Used in sedes.php, creates a list with all the existing campuses.
	...
	//Used in resources.php, generates a table with all the existing resources.
	...
	public static function datosTodasReservas($reservas, $max, $page) {
		...
		$tabla->head = array (
				...
				get_string ( 'student', 'local_reservasalas' ),
				get_string ( 'module', 'local_reservasalas' ),
				get_string ( 'state', 'local_reservasalas' ),
				get_string ( 'confirmedfrom', 'local_reservasalas' ),
				"PC",
				get_string ( 'reservecomment', 'local_reservasalas' ),
		...
		foreach ( $reservas as $reserva ) {
			...
			if ($count <= $page * $max && $count > ($page - 1) * $max) {
				...
				$alumno = $DB->get_record ( 'user', array (
				...
				$editurl_sala = new moodle_url ( 'historial.php', array (
						'action' => 'comentario',
						'reserva' => $reserva->id 
				) );
	...
	public static function searchRooms($data) {
		...
		$table->head = array (
				...
				get_string ( 'module', 'local_reservasalas' ),
				html_writer::checkbox ( "", "0", false, '', array (
						'onClick' => 'checkAll()',
						'id' => 'check' 
				) ) 
		);
		...
		foreach ( $data as $info ) {
			...
			$table->data [] = array (
					...
					html_writer::checkbox ( "check_list[" . $i . "]", $info->id, false, '', array (
							'class' => 'check' 
					) ) 
			);
			$i ++;
	...
	public static function getInfo($h, $idsala, $repetir, $idmodulo, $idmoduloA = null) { // $reserve, $module, $creation, $active){
		...
		$tabla->head = array (
				get_string ( 'date', 'local_reservasalas' ),
				get_string ( 'event', 'local_reservasalas' ),
				get_string ( 'assistants', 'local_reservasalas' ),
				get_string ( 'usercharge', 'local_reservasalas' ),
				get_string ( 'module', 'local_reservasalas' ) 
		);
		...
		foreach ( $datas as $data ) {
			...
			$module = $DB->get_record ( 'reservasalas_modulos', array (
					'id' => $data->modulo 
			) );
	...
	public static function getMail($n, $messageRoom, $messageDate, $messageModule, $messageCap) {
		...
		$tabla->head = array (
				...
				get_string ( 'module', 'local_reservasalas' ),
				get_string ( 'capacity', 'local_reservasalas' ) 
		);// date, campus, building, room, capacity. 
		if ($n != 0) {
	...
	public static function getList() { // not tested
		global $DB;
		$tabla = new html_table ();
		$planillas = $DB->get_records ( 'reservasalas_planillas' );
		$tabla->head = array (
				get_string ( 'namelist', 'local_reservasalas' ),
				get_string ( 'adjustments', 'local_reservasalas' ) 
		);
		if (! empty ( $planillas )) {
			foreach ( $planillas as $planilla ) {
				$deleteurl_resource = new moodle_url ( 'modulos.php', array (
						'action' => 'borrar',
						'idresource' => $resource->id,
						'sesskey' => sesskey () 
				) );
		...
		return $tabla; // if returns a empty table creates a button to create modules
	}
}