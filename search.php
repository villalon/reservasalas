...
// Capability: reserve, modify, cancel, consult.
...
>>>>>>> 01637c4100075e77eadcc873b22be1e814218720
		...
		if (empty ( $result ) || $condition == 1) { // $condition=1 means there are no rooms.
			...
			echo html_writer::tag ( '<form', '', array (
					'name' => 'search',
					'method' => 'POST' 
			) );
	...
	foreach ( $check_list as $check ) {
		...
		$data = $DB->get_record ( 'reservasalas_reservas', array (
				'id' => $check 
		) );
		$room = $DB->get_record ( 'reservasalas_salas', array (
				'id' => $data->salas_id 
		) );
		$building = $DB->get_record ( 'reservasalas_edificios', array (
				'id' => $room->edificios_id 
		) );
		$campus = $DB->get_record ( 'reservasalas_sedes', array (
				'id' => $building->sedes_id 
		) );
		$module = $DB->get_record ( 'reservasalas_modulos', array (
				'id' => $data->modulo 
		) );
		$responsable = $DB->get_record ( 'user', array (
				'id' => $data->alumno_id 
		) );
		$table->data [] = array (
				$campus->nombre,
				...
				$data->nombre_evento,
				$data->fecha_reserva,
				date ( "Y-m-d", $data->fecha_creacion ),
	...
	if ($fromform = $form->get_data ()) {
		...
		$sala = $DB->get_record ( 'reservasalas_salas', array (
				'nombre' => $fromform->name,
				'edificios_id' => $fromform->campus 
		) );
